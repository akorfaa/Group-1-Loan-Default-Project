...
st.markdown('Predict whether a loan applicant will default based on their profile and loan history.')
...
with tab1:
    ...
    with col1:
        ...
        is_referred  = st.selectbox('Referred by another customer?', [0, 1],
                                     format_func=lambda x: 'Yes' if x == 1 else 'No')
    with col2:
        bank_account_type         = st.selectbox('Bank Account Type', ['Savings', 'Current', 'Unknown'])
        employment_status_clients = st.selectbox('Employment Status',
                                                  ['Permanent', 'Self-Employed', 'Unemployed', 'Student', 'Unknown'])
        bank_name_clients         = st.selectbox('Bank Name', ['GTBank', 'Access', 'First Bank', 'Zenith', 'UBA', 'Unknown'])
    ...
    if st.button('Predict Default Risk', type='primary'):
        input_data = {
            'loannumber': loannumber, 'loanamount': loanamount,
            'totaldue': totaldue, 'termdays': termdays,
            'days_to_approve': 1, 'interest_amount': totaldue - loanamount,
            'interest_rate': (totaldue - loanamount) / loanamount,
            'is_referred': is_referred, 'approval_month': approval_month,
            'approval_year': approval_year, 'bank_account_type': bank_account_type,
            'longitude_gps': 3.3792, 'latitude_gps': 6.5244,
            'bank_name_clients': bank_name_clients,
            'employment_status_clients': employment_status_clients,
            'age': age, 'prev_num_loans': prev_num_loans,
            'prev_avg_loan_amount': loanamount, 'prev_total_borrowed': loanamount * prev_num_loans,
            'prev_avg_totaldue': totaldue, 'prev_avg_termdays': termdays,
            'prev_max_loannumber': loannumber, 'prev_avg_interest_rate': (totaldue - loanamount) / loanamount,
            'prev_avg_loan_duration': termdays, 'prev_late_payments': prev_late_payments,
            'prev_late_payment_rate': prev_late_payment_rate, 'prev_avg_days_to_repay': 0.0
        }
...
with tab2:
    ...
    if uploaded:
        ...
        if st.button('Run Predictions', type='primary'):
            ...
            results['Label'].value_counts().plot(kind='bar', ax=ax, color=['steelblue', 'tomato'], edgecolor='black')
